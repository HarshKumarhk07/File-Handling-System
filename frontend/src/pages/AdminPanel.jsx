...
import { FaTrash, FaUser, FaFile, FaArrowUp, FaArrowDown, FaEye } from 'react-icons/fa';
            ...
            <div className="container mx-auto p-4 sm:p-6">
                ...
                <div className="flex gap-4 mb-6 border-b border-gray-700 pb-2 overflow-x-auto">
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded transition whitespace-nowrap ${activeTab === 'files' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'}`}
                    >
                        <FaFile /> Files ({stats.totalFiles})
                    </button>
                    <button
                        onClick={() => setActiveTab('users')}
                        className={`flex items-center gap-2 px-4 py-2 rounded transition whitespace-nowrap ${activeTab === 'users' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'}`}
                    >
                                ...
                                <tbody className="divide-y divide-gray-700">
                                    {files.map((file, index) => (
                                        <tr key={file._id} className="hover:bg-gray-750">
                                            <td className="p-4 text-gray-400 font-mono text-sm">{index + 1}</td>
                                            <td className="p-4 truncate max-w-xs" title={file.originalName}>{file.originalName}</td>
                                            <td className="p-4 whitespace-nowrap">{file.owner?.email || 'Unknown'}</td>
                                            <td className="p-4 text-sm whitespace-nowrap">{file.size >= 1024 * 1024 ? `${(file.size / 1024 / 1024).toFixed(2)} MB` : `${(file.size / 1024).toFixed(2)} KB`}</td>
                                            <td className="p-4 text-xs text-gray-400 whitespace-nowrap">{file.mimetype}</td>
